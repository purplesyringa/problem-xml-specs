...
In addition to all operations supported by files, `submission` has the `submission.rate` method, which is described below, and the `submission.metadata` property, which is a dictionary of implementation-defined data. Common fields are `id` for submission ID, `author` for the username of the author of the submission, and `timestamp` for UNIX timestamp of the submission time.
